...
public class PostBodyRequestWrapper extends HttpServletRequestWrapper {
    ...
    private synchronized void buildContentData() {
        try {
            ...
            // 设置版本相关信息
            if (ctx.get("client") != null) {
                dataParam.put("client", ctx.get("client"));
            }
            if (ctx.get("version") != null) {
                dataParam.put("version", ctx.get("version"));
            }
            if (ctx.get("channelCode") != null) {
                dataParam.put("channelCode", ctx.get("channelCode"));
            }
            // 设置分页信息
